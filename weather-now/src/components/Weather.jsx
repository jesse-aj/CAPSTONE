...
//  This code takes and holds all states that are to be displayed
const Weather = () => {
  ...
  //This renders when the app starts (To search and fetch for weather data)
    ...
    const loadDefaultWeather = async () => {
        try {
            ...
            setWeather(data);
                 ...
                 //This also shows the 5 day forcaste of the default weather which is Accra or any city searched
           ...
           item.dt_txt.includes("12:00:00")      //This trim the weather to return info from the API from 12 midnight, instead of taking it every 3hrs
                );
  ...
  const handleSearch = async () => {
    ...
    if (!city.trim()) return;       //This simply trims the text to exclude any space left     
    setLoading(true);
    setError("");
    setWeather(null);
    setForcast([]);
...
const handleRefresh = async () => {
  ...
  try {
    ...
    //This is a simple condition that  checks and filters the 5 day forcast
    if (forcastData && forcastData.list) {
      const dailyData = forcastData.list.filter((item) =>
        item.dt_txt.includes("12:00:00")
      );
    ...
    //This handles any refresh error
  } catch (err) {
  ...
  return (
    <div>
      ...
      {!loading && <ErrorMessage message={error} />}
               ...
               <div className="flex justify-between items-start w-screen h-screen bg-gray-50
      ...
      {weather && <WeatherCard weather={weather} units={units}/>}
               ...
               <div className=" w-[380px] h-full bg-white/60 dark:bg-white/10 backdrop-blur-2xl
                shadow-xl border-l border-gray-200 dark:border-gray-700 p-6 overflow-y-auto">
